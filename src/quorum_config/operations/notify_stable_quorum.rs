...
impl Operation for NotifyQuorumOperation {
    ...
    fn iterate<NT>(&mut self, node: &mut InternalNode, network: &NT) -> atlas_common::error::Result<OperationResponse> where NT: QuorumConfigNetworkNode + 'static {
        ...
        match &self.current_state {
            OperationState::Waiting => {
                ...
                node.node_type().quorum_communication().send(QuorumReconfigurationMessage::ReconfigurationProtocolStable(view_members))?;
    ...
    fn handle_received_message<NT>(&mut self, node: &mut InternalNode, network: &NT, header: Header, message: OperationMessage) -> atlas_common::error::Result<OperationResponse> where NT: QuorumConfigNetworkNode + 'static {
        Ok(OperationResponse::Processing)
    }

    fn handle_quorum_response<NT>(&mut self, node: &mut InternalNode, network: &NT, quorum_response: QuorumReconfigurationResponse) -> atlas_common::error::Result<OperationResponse> where NT: QuorumConfigNetworkNode + 'static {
        return match self.current_state {
            OperationState::Waiting => {
                unreachable!("We should not be waiting for a quorum response at this point")
            }
            OperationState::WaitingQuorumResponse => {
                match quorum_response {
                    ...
                    _ => {
                        Ok(OperationResponse::AwaitingResponseProtocol)
                    }
                }
            }
            OperationState::Done => {
    ...
    fn finish<NT>(self, observer: &mut InternalNode, network: &NT) -> atlas_common::error::Result<()> where NT: QuorumConfigNetworkNode + 'static {
        Ok(())
    }
}